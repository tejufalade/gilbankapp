...
const BankOffers = () => {
  return (
    <div className='w-screen h-[82vh] px-8 py-8 bg-white'>
        <h2 className='text-[50px] text-navyblue font-semibold text-center'>What we offer</h2>
        <div className='flex justify-evenly items-center w-full'>
          {cards.map((card)=>(
                <div className='h-[550px] w-[420px] bg-navyblue mt-4 text-white py-4 flex flex-col items-center align-center px-8 text-[20px]' key={card.title}>
                <img src={card.img} alt={card.title} className='w-16 h-16 mt-16'/>
                <h6 className='mt-8 font-light w-2/3 text-center'>{card.title} </h6>
                <p className='mt-8 text-center'>{card.benefits} </p>
                <p className='mt-8 font-light text-center'>{card.description} </p>
